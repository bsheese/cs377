...
To solve this, we can compute a new metric called **Variance**, which takes the TSS and divides it by the size of the sample. The variance is simply the average of the squared deviations. For Sample A, the variance is 0. For Sample B, the variance is 36 (7,200 / 200), which is the exact same variance we would have calculated with our original sample of 100 ($3,600 / 100 = 36$). 

We can take this one step further. Because variance is measured in squared units (squared inches), we can take the square root of the variance to put our metric back into our original unit of measure. This metric is called the **Standard Deviation**. For Sample B, the variance is 36 squared inches; the square root of 36 is 6 inches. This tells us that, on average, students in Sample B deviate from the mean by 6 inches. 
...
Same goal, we're going to estimate an unseen student's height. We have data on the heights of 100 other students from the same school. But this time, we have some additional information. We know the shoesize of the unseen student, and we know both the height and the shoesize of the 100 other students. 
...
To breifly recap, we will take one of variables, shoe size in this case, we will make a list of all the shoe sizes of the students in our sample. We will then subtract the mean shoe from those values and produce a list of deviations. We will so the same thing for height and we will wind up with a deviation for height and deviation for show size for each student. 

We can calssify our student into one of four groups:

* Taller than average, bigger feet than average: These student will a positve deviation for both height and shoe size. 

* Shorter than average, smaller feet than average: These students will have a negative deviation for both height and shoe size. 

* Taller than average, smaller feet than average: These students will have a positive deviation for height and a negative deviation for shoe size. 

* Shorter than average, bigger feet than average: These student will have a negative deviation for hegith and a positive deviation for shoe size. 

Now, imagine we plot these into four quadrants with height on the x axis and show size on the y axis. The mean for height is used to cut the x-axis in half, and the mean for shoe size is used to split the y axis. If we were right about a positive relation between height and shoe size what we expect the data in these quadrants to look like? Just eye-balling it, it seems like we should expect more students in the Tall/Big and the Short/Small quadrants. However, what would it mean if we counted up the number of students in each quadrant and found they were equal? If number of Taller/Smaller + Shorter/Bigger is roughly equal to the Taller/Bigger + Shorter/Smaller count we might start to worry about our hypothesis. 

We can do a bit more calculation to actually settle the issue. If we muliply the deviation of height by the deviation of shoesize for each participant we will end up with positive values for students in Taller/Bigger quadrant and the Shorter/Smaller quadrant. In contrast, we wind up with negative values for students in the Taller/Smaller and Smaller/Bigger quadrants. 
