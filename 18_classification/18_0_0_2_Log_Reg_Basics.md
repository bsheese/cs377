...
Now that we've covered the basics of logistic regression and how it predicts probabilities for binary outcomes, it's time to dig into the mathematical details that make logistic regression work. In this section, we'll explore the key mathematical concepts that underlie logistic regression: probability, odds, the logistic (sigmoid) function, and the logit transformation.
...
To maximize the log-likelihood function, we need an optimization algorithm that can iteratively adjust the coefficients until we reach the maximum point. One of the most commonly used optimization algorithms in logistic regression is **gradient descent**.

#### **Gradient Descent**

Gradient descent is an optimization algorithm that finds the minimum or maximum of a function by iteratively moving in the direction of the steepest ascent or descent. In logistic regression, we use gradient descent to adjust the coefficients to maximize the log-likelihood function.
...
While gradient descent is a popular method for optimizing the log-likelihood in logistic regression, other techniques such as **Newton-Raphson** and **stochastic gradient descent (SGD)** are also used, especially for larger datasets. These methods can offer faster convergence or be more efficient in certain cases, but the core idea remains the same: iteratively adjusting the coefficients to maximize the likelihood of the observed data.
