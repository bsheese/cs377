...
### Searching for the Sage family with df['Name'].str.contains('Sage') also returns 'Mlle. Emma Sagesser'. In this dataset names end with the surname (e.g., 'Mr. Frederick Sage'). Which approach correctly excludes her?
- [x] Combine masks: str.contains('Sage') & ~str.contains('Sagesser')
- [ ] Use str.startswith('Sage ') to require a word boundary at the start
- [ ] Append a wildcard: str.contains('Sage*')
- [ ] Use str.upper() on the names before searching so the match is exact
